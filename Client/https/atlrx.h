...
template <typename RECHARTYPE = char> class CAToREChar {
  public:
    CAToREChar(const char *psz) throw() : m_psz(psz) {}
    operator const RECHARTYPE *() const throw() { return m_psz; }
    const char *m_psz;
};

template <> class CAToREChar<wchar_t> {
  public:
    CAToREChar(const char *psz) throw() : m_a2w(psz) {}
...
class CAtlRECharTraitsA {
  public:
    ...
    static RECHARTYPE *Next(const RECHARTYPE *sz) throw() { return (RECHARTYPE *)(sz + 1); }
    ...
    static int Strnicmp(const RECHARTYPE *szLeft, const RECHARTYPE *szRight,
                        size_t nCount) throw() {
...
#pragma warning(pop)
    ...
    static int Isdigit(RECHARTYPE ch) throw() { return isdigit(static_cast<unsigned char>(ch)); }

    static const RECHARTYPE **GetAbbrevs() {
        static const RECHARTYPE *s_szAbbrevs[] = {"a([a-zA-Z0-9])", // alpha numeric
                                                  "b([ \\t])",      // white space (blank)
                                                  "c([a-zA-Z])",    // alpha
                                                  "d([0-9])",       // digit
                                                  "h([0-9a-fA-F])", // hex digit
    ...
    static BOOL UseBitFieldForRange() throw() { return TRUE; }

    static int ByteLen(const RECHARTYPE *sz) throw() { return int(strlen(sz)); }
};
...
#pragma warning(pop)
    ...
    static int Isdigit(RECHARTYPE ch) throw() { return iswdigit(ch); }

    static const RECHARTYPE **GetAbbrevs() {
        static const RECHARTYPE *s_szAbbrevs[] = {L"a([a-zA-Z0-9])", // alpha numeric
    ...
    static BOOL UseBitFieldForRange() throw() { return FALSE; }

    static int ByteLen(const RECHARTYPE *sz) throw() { return int(wcslen(sz) * sizeof(WCHAR)); }
};
...
class CAtlRECharTraitsMB {
  public:
    ...
    static RECHARTYPE *Next(const RECHARTYPE *sz) throw() { return _mbsinc(sz); }
...
#pragma warning(pop)
    ...
    static int Isdigit(RECHARTYPE ch) throw() { return _ismbcdigit((unsigned int)ch); }
    ...
    static int ByteLen(const RECHARTYPE *sz) throw() { return (int)strlen((const char *)sz); }
};
...
template <class CharTraits = CAtlRECharTraits> class CAtlRegExp; // forward declaration
...
template <class CharTraits = CAtlRECharTraits> class CAtlREMatchContext {
  ...
  protected:
    ...
    BOOL Push(size_t n) { return Push((void *)n); }
...
template <class CharTraits /* =CAtlRECharTraits */> class CAtlRegExp {
  public:
    ...
    BOOL Match(const RECHAR *szIn, CAtlREMatchContext<CharTraits> *pContext,
...
#pragma warning(disable : 4127) // conditional expression is constant
        ...
        while (1) {
            ...
            case RE_PREVIOUS: {
                BOOL bMatch = FALSE;
                if (m_bCaseSensitive) {
                    bMatch = !CharTraits::Strncmp(
                        sz, pContext->m_Matches[GetInstruction(ip).prev.nGroup].szStart,
                        pContext->m_Matches[GetInstruction(ip).prev.nGroup].szEnd -
                            pContext->m_Matches[GetInstruction(ip).prev.nGroup].szStart);
                } else {
                    bMatch = !CharTraits::Strnicmp(
                        sz, pContext->m_Matches[GetInstruction(ip).prev.nGroup].szStart,
                        pContext->m_Matches[GetInstruction(ip).prev.nGroup].szEnd -
                            pContext->m_Matches[GetInstruction(ip).prev.nGroup].szStart);
                }
                if (bMatch) {
                    sz += pContext->m_Matches[GetInstruction(ip).prev.nGroup].szEnd -
                          pContext->m_Matches[GetInstruction(ip).prev.nGroup].szStart;
                    ip++;
                    break;
                }
            ...
            case RE_PUSH_GROUP:
                pContext->Push(
                    (void *)pContext->m_Matches[GetInstruction(ip).group.nGroup].szStart);
                pContext->Push((void *)pContext->m_Matches[GetInstruction(ip).group.nGroup].szEnd);
                ip++;
                break;

            case RE_POP_GROUP:
                pContext->m_Matches[GetInstruction(ip).group.nGroup].szEnd =
                    (const RECHAR *)pContext->Pop();
                pContext->m_Matches[GetInstruction(ip).group.nGroup].szStart =
                    (const RECHAR *)pContext->Pop();
                ip++;
                break;
...
#pragma warning(pop) // 4127
  ...
  protected:
    ...
    REParseError GetLastParseError() throw() { return m_LastError; }

    void SetLastParseError(REParseError Error) throw() { m_LastError = Error; }
    // CAtlRegExp::Reset
    ...
    INSTRUCTION &GetInstruction(size_t nIndex) throw() { return m_Instructions[nIndex]; }
    ...
    int ParseE(const RECHAR **ppszRE, bool &bEmpty) {
        ...
        if (type == RE_STAR_BEGIN || type == RE_QUESTION || type == RE_NG_STAR_BEGIN ||
            type == RE_NG_QUESTION) {
    ...
    // This function fix these pointers to point into the original, user supplied buffer (first
    // param to Match method). Example: If a ptr (szStart) currently point to <internal buffer>+3, it
    // is fixed to <user supplied buffer>+3
    void FixupMatchContext(CAtlREMatchContext<CharTraits> *pContext, const RECHAR *szOrig,
                           const RECHAR *szNew) {
        ATLENSURE(pContext);
        ATLASSERT(szOrig);
        ATLASSERT(szNew);
  ...
  public:
    ...
    virtual void OnDebugEvent(size_t ip, const RECHAR *szIn, const RECHAR *sz,
