 ...
 *  [in] allowDuplicate: Value of type bool which indicates whether duplicate items are added to the
 * output result.
 *
 * Return Value
 ...
 *
 * Remarks
 *  The output result is devided into groups.  User should get the groups according to the group
 * count.  For example:
 *  1. RegExp = L"{ab}", source = L"abcabe", then result = L"ab", L"ab".
 *  2. RegExp = L"{ab}{cd}", source = L"abcdeabecd", then result = L"ab", L"cd", L"ab", L"cd".
 */
inline bool ParseRegExp(const wstring &regExp, bool caseSensitive, int groupCount,
                        const wstring &source, vector<wstring> &result,
        ...
        if (bSucceed) {
            ...
            if (!allowDuplicate) {
                ...
                for (vector<wstring>::iterator it = result.begin(); it != result.end();) {
                    bool bEqual = true;
                    for (vector<wstring>::iterator tempMatchIt = tempMatch.begin();
                         tempMatchIt != tempMatch.end(); tempMatchIt++, it++) {
            ...
            if (bAdd) {
                for (vector<wstring>::iterator tempMatchIt = tempMatch.begin();
                     tempMatchIt != tempMatch.end(); tempMatchIt++) {
