...
inline bool SeparateString(const wstring &content, const wstring &delimiter,
                           vector<wstring> &result) {
...
inline wstring URLEncoding(const wstring &keyword, bool convertToUTF8 = true) {
    ...
    if (convertToUTF8) {
        iLength = ::WideCharToMultiByte(CP_UTF8, 0, keyword.c_str(), keyword.length(), NULL, 0,
                                        NULL, NULL);
        if (iLength <= 0) {
            return L"";
        }
        ...
        iLength = ::WideCharToMultiByte(CP_UTF8, 0, keyword.c_str(), keyword.length(), szKeyword,
                                        iLength, NULL, NULL);
    } else {
...
inline unsigned int GetKeywordMatchGrade(const wstring &source, const wstring &keyword) {
    ...
    if (SeparateString(keyword, L" ", results) && results.size() > 0) {
        ...
        for (vector<wstring>::size_type index = 0; index < results.size(); index++) {
            grade += GetSeparateKeywordMatchGrade(src, results[index]) * results[index].length() /
                     keywordTotalLength;
        }
    } else {
...
inline wstring GetDateString(const COleDateTime &time, const wstring &separator = L"-",
                             bool align = true) {
    wstring date = L"";
...
inline wstring GetTimeString(const COleDateTime &time, const wstring &separator = L":",
                             bool align = true) {
    wstring date = L"";
    wchar_t szTemp[MAX_PATH] = L"";
