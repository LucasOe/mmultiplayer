...
static const int INT_BUFFERSIZE =
    10240; // Initial 10 KB temporary buffer, double if it is not enough.
...
class WinHttpClient {
  public:
    ...
    inline bool SetTimeouts(unsigned int resolveTimeout = 0, unsigned int connectTimeout = 60000,
                            unsigned int sendTimeout = 30000, unsigned int receiveTimeout = 30000);
...
WinHttpClient::WinHttpClient(const wstring &url, PROGRESSPROC progressProc)
    : m_requestURL(url), m_sessionHandle(NULL), m_requireValidSsl(false), m_responseHeader(L""),
      m_responseContent(L""), m_responseCharset(L""), m_requestHost(L""), m_pResponse(NULL),
      m_responseByteCountReceived(0), m_pfProcessProc(progressProc), m_responseByteCount(0),
      m_responseCookies(L""), m_additionalRequestCookies(L""), m_pDataToSend(NULL),
      m_dataToSendSize(0), m_proxy(L""), m_dwLastError(0), m_statusCode(L""), m_userAgent(L"WinHttpClient"),
      m_bForceCharset(false), m_proxyUsername(L""), m_proxyPassword(L""), m_location(L""),
      m_resolveTimeout(0), m_connectTimeout(60000), m_sendTimeout(30000), m_receiveTimeout(30000) {}
...
bool WinHttpClient::SendHttpRequest(const wstring &httpVerb, bool disableAutoRedirect) {
    ...
    if (m_sessionHandle == NULL) {
        m_sessionHandle = ::WinHttpOpen(m_userAgent.c_str(), WINHTTP_ACCESS_TYPE_DEFAULT_PROXY,
    ...
    ::WinHttpSetTimeouts(m_sessionHandle, m_resolveTimeout, m_connectTimeout, m_sendTimeout,
                         m_receiveTimeout);
        ...
        if (hConnect != NULL) {
            ...
            if (hRequest != NULL) {
                ...
                if (!m_requireValidSsl && urlComp.nScheme == INTERNET_SCHEME_HTTPS) {
                    ...
                    ::WinHttpSetOption(hRequest, WINHTTP_OPTION_SECURITY_FLAGS, (LPVOID)&options,
                                       sizeof(DWORD));
                }
                ...
                while (!bGetReponseSucceed && iRetryTimes++ < INT_RETRYTIMES) {
                    if (m_additionalRequestHeaders.size() > 0) {
                        if (!::WinHttpAddRequestHeaders(
                                hRequest, m_additionalRequestHeaders.c_str(),
                    ...
                    if (m_additionalRequestCookies.size() > 0) {
                        ...
                        if (!::WinHttpAddRequestHeaders(
                                hRequest, cookies.c_str(), cookies.size(),
                                WINHTTP_ADDREQ_FLAG_COALESCE_WITH_SEMICOLON)) {
                            m_dwLastError = ::GetLastError();
                        }
                    }
                    if (m_proxy.size() > 0) {
                        ...
                        if (!::WinHttpSetOption(hRequest, WINHTTP_OPTION_PROXY, &proxyInfo,
                                                sizeof(proxyInfo))) {
                            m_dwLastError = ::GetLastError();
                        }
                    ...
                    if (::WinHttpSendRequest(hRequest, WINHTTP_NO_ADDITIONAL_HEADERS, 0,
                        ...
                        if (::WinHttpGetIEProxyConfigForCurrentUser(&proxyConfig)) {
                            if (proxyConfig.lpszAutoConfigUrl != NULL) {
                                ...
                                if (::WinHttpGetProxyForUrl(m_sessionHandle, m_requestURL.c_str(),
                                                            &autoProxyOptions, &proxyInfo)) {
                                    if (::WinHttpSetOption(hRequest, WINHTTP_OPTION_PROXY,
                    ...
                    if (bSendRequestSucceed) {
                        if (m_pDataToSend != NULL) {
                            DWORD dwWritten = 0;
                            if (!::WinHttpWriteData(hRequest, m_pDataToSend, m_dataToSendSize,
                                                    &dwWritten)) {
                                m_dwLastError = ::GetLastError();
                            }
                        }
                        if (::WinHttpReceiveResponse(hRequest, NULL)) {
                                ...
                                if (szHeader != NULL) {
                                    ...
                                    if (::WinHttpQueryHeaders(
                                        ...
                                        if (!m_bForceCharset) {
                                            ...
                                            if (ParseRegExp(regExp, false, 1, m_responseHeader,
                                                            result) &&
                                                result.size() > 0) {
                                        ...
                                        if (ParseRegExp(regExp, false, 1, m_responseHeader,
                                            ...
                                            result.size() > 0) {
                                            m_responseByteCount =
                                                (unsigned int)_wtoi(result[0].c_str());
                                        }
                                        ...
                                        if (ParseRegExp(regExp, false, 1, m_responseHeader,
                                                        result) &&
                                            result.size() > 0) {
                                            for (vector<wstring>::size_type i = 0;
                                                 i < result.size(); i++) {
                                            ...
                                            if (m_responseCookies.size() > 0 &&
                                                m_responseCookies[m_responseCookies.size() - 1] !=
                                                    L';') {
                                                m_responseCookies += L";";
                                            }
                                        }
                            ...
                            dwSize = 0;
                            bResult = ::WinHttpQueryHeaders(hRequest, WINHTTP_QUERY_STATUS_CODE,
                            ...
                            do {
                                ...
                                if (::WinHttpQueryDataAvailable(hRequest, &dwSize)) {
                                    ...
                                    if (pResponse != NULL) {
                                        ...
                                        DWORD dwRead = 0;
                                        if (::WinHttpReadData(hRequest, pResponse, dwSize,
                                                              &dwRead)) {
                                            ...
                                            memcpy(m_pResponse + iCurrentBufferSize, pResponse,
                                                   dwRead);
                            ...
                            int iLength =
                                ::MultiByteToWideChar(codePage, dwFlag, (LPCSTR)m_pResponse,
                                                      m_responseByteCountReceived + 1, NULL, 0);
                            if (iLength <= 0) {
                            ...
                            if (iLength > 0) {
                                ...
                                if (wideChar != NULL) {
                                    ...
                                    iLength = ::MultiByteToWideChar(
                                        codePage, dwFlag, (LPCSTR)m_pResponse,
                                        m_responseByteCountReceived + 1, wideChar, iLength);
                                    if (iLength > 0) {
                            ...
                            if (verb == L"GET" && !disableAutoRedirect) {
                                ...
                                vector<wstring> result;
                                if (ParseRegExp(regExp, false, 1, m_responseContent, result) &&
                                    result.size() > 0) {
                                    regExp = L"{</html>}";
                                    if (!ParseRegExp(regExp, false, 1, m_responseContent, result) ||
                                        result.size() <= 0) {
...
wstring WinHttpClient::GetResponseHeader(void) { return m_responseHeader; }

wstring WinHttpClient::GetResponseContent(void) { return m_responseContent; }

wstring WinHttpClient::GetResponseCharset(void) { return m_responseCharset; }

wstring WinHttpClient::GetRequestHost(void) { return m_requestHost; }
...
wstring WinHttpClient::GetResponseCookies(void) { return m_responseCookies; }
...
const BYTE *WinHttpClient::GetRawResponseContent(void) { return m_pResponse; }

unsigned int WinHttpClient::GetRawResponseContentLength(void) { return m_responseByteCount; }

unsigned int WinHttpClient::GetRawResponseReceivedContentLength(void) {
    return m_responseByteCountReceived;
}

DWORD WinHttpClient::GetLastError(void) { return m_dwLastError; }

wstring WinHttpClient::GetResponseStatusCode(void) { return m_statusCode; }
...
wstring WinHttpClient::GetResponseLocation(void) { return m_location; }

bool WinHttpClient::SetTimeouts(unsigned int resolveTimeout, unsigned int connectTimeout,
                                unsigned int sendTimeout, unsigned int receiveTimeout) {
